...
public class SampledTripartiteEmbeddingViaWord2Vec {
    ...
    public List<Double> train(List<String> fileList, int n_walks, int n_nodes, int length) {
        try {
            SampledMetaAlgorithm meta = new SampledMetaAlgorithm();
...
//        nodes.add("1");
    ...
    /**
     * FIXME:具体使用时候需要改变正则匹配的函数，根据规则筛选
    ...
    public Map<String, List<Double>> getRandom_K_Embeddings(int k){
        ...
        // FIXME:进行正则匹配，只保留符合sample_的，也就是样本点
        nodeList = nodeListTemp.stream().filter(this::judgeTupleWithoutSource).collect(Collectors.toList());
...
//        }
    ...
    private boolean judgeTuple(String str){
        String regex = "row_([0-9])_y.";
        return Pattern.matches(regex,str);
    }
    private boolean judgeTupleWithoutSource(String str){
        String regex = "sample_.";
        return Pattern.matches(regex,str);
    }
