package main.java.iatd.weather.origin;
...
public class GA_iatd_origin extends GeneticAlgorithm {
...
//            Collections.sort(rmseList);
    ...
    public List<Double> errorForMonitor(String[][] calcTruth, String[][] goldenStandard,int D1,int D2){
        ...
        double sum = 0;
        // attention : fixcsy change source 更换数据集后，根据列的多值情况，num多值或者string类型多值，更改else if的判断条件!!!
        for(int i = 0;i<D1;i++){
            for(int j = 2;j<D2;j++){
                int csy2 = 0;
...
//            truthFilePath = "data/dart/monitor/monitor_truth.csv";
    ...
    @Test
    public void test(){
        GA_iatd_origin gaImplTest = new GA_iatd_origin();
