...
# 设置数据的参数
all_num = 95  # 所有数据的数目（包括首行）
source_num = 5
domain_num = 4
object_number = 20
max_claim = 100
name_index, domain_index, source_index, author_index = 0, 2, 1, 4
book_domain = []
domain = []
book_object = []
...
def read_data():
    data = []
    with open("monitor_original.csv", 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            data.append(row)
    return data
        ...
        with open(file, 'r') as embedding:
            for row in embedding:
                if kk == 0:
                    ...
                    if word2 == 'iiyama_North_America':
                        word2 = 'iiyama North America'
                    if word3 == 'iiyama_North_America':
                    ...
                    if word3 == 'Hannspree':
                        word3 = 'Hannspree'
                    print(domain)
                    print(word1, domain.index(word2)+1, domain.index(word3)+1, len(infs[1][1]))
                    infs[word1][domain.index(word2)+1][domain.index(word3)+1] = float(number)
                kk += 1
    return infs, name


def one_step(book, book_domain, file, flag):
    # 算Pd(s)[domain][s]
    ...
    # 算cs(v)
    cs = [[[[0 for _ in range(source_num+1)] for i in range(max_claim+1)] for j in range(object_number+1)] for k in range(source_num+1)]  # 现根据value定位v[v,source]
    # 算每个对象对应的所有值以及源
    # value[s][object][name, author]
    value = [[[0 for k in range(max_claim + 1)] for i in range(object_number + 1)] for j in range(source_num+1)]
    j = 0
    for i in range(1, all_num):
        if book[i][name_index] != book[i - 1][name_index]:
            ...
            for m in range(1, source_num+1):
                if m == int(book[i][source_index]):
                    value[m][j][name_index] = book[i][name_index]
                    break
            temp = ''
            k = 0
            t = 0
    ...
    for i in range(1, object_number+1):
        ...
        for j in range(1, source_num+1):
            for k in range(1, max_claim+1):
                if ov[j][i][k] > 0.22 and value[j][i][k] != 0:
        ...
        if flag == 0:
            print(ov[1][i], "\n", ov[2][i], "\n", ov[3][i], "\n", ov[4][i], "\n", ov[5][i])
        '''
        output.append([value[1][i][0]]+[already])
        already = []
    name = name[:-1]
    f = open(name+"_truth_pro.csv", 'w', newline="")
...
def score(a, b):
    truth = []
    with open('camera_truth.csv', 'r') as f:
        reader = csv.reader(f)
        for row in reader:
            truth.append(row)
...
def run(file, flag):
    ...
    data_process(book)
    print("data:", book)
    a, name = one_step(book, book_domain, file, flag)
    end = time.time()
